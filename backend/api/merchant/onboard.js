...
export async function POST(req) {
  ...
  if (merchantErr) {
    return json({ error: `Merchant configured on-chain, but merchant profile failed to save: ${merchantErr.message}` }, 500);
  }
  ...
  if (products.length > 0) {
    const rows = products.slice(0, 50).map(p => ({
      ...
      price: String(BigInt(p.price)), // USDC 6-dec, bigint-as-text
    ...
    if (catalogErr) {
      return json({ error: `Merchant configured on-chain, but catalog items failed to save: ${catalogErr.message}` }, 500);
    }
  }
